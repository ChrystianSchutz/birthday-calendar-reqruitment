...
class Calendar extends React.Component {
  componentDidMount() {
    this.props.fetchData('/birthdays')
  }
  ...
  renderCells() {
    const weekStart = dateFns.startOfWeek(this.props.getCurrentWeek)
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const cloneDay = day
      const dayOfWeek = startingDayOfWeek + i - 1
      days.push(
        <div
          className={`col cell ${
            !dateFns.isSameMonth(day, weekStart)
              ? 'disabled'
              : dateFns.isSameDay(day, this.props.getSelectedDate)
                ? 'selected'
                : ''
          }`}
          key={day}
  ...
  nextWeek = () => {
    this.props.setCurrentWeek(dateFns.addWeeks(this.props.getCurrentWeek, 1))
  }

  prevWeek = () => {
    this.props.setCurrentWeek(dateFns.subWeeks(this.props.getCurrentWeek, 1))
  }
